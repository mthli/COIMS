...
int main(int argc, char **argv)
{
    ...
    */
    ...
    (*item).get_column = -1;
    g_signal_connect(
            G_OBJECT((*item).hospital_list),
            "select-row",
            G_CALLBACK(get_row_and_column),
            item
            );
    ...
    (*item).num_entry = gtk_entry_new();
    gtk_widget_set_size_request((*item).num_entry, 150, 25);
    gtk_table_attach(
            GTK_TABLE(temp_table),
            (*item).num_entry,
            4, 5,
    ...
    (*item).days_entry = gtk_entry_new();
    gtk_widget_set_size_request((*item).days_entry, 150, 25);
    gtk_table_attach(
            GTK_TABLE(temp_table),
            (*item).days_entry,
            2, 3,
    ...
    temp_button = gtk_button_new_with_label("确定");
    g_signal_connect(
            G_OBJECT(temp_button),
            "clicked",
            G_CALLBACK(check_sure),
            item
            );
